...
namespace rti_utils {
    ...
    // TODO: снаружи везде это приводится пока к TRtiTime
    inline double convertTime( const std::wstring & _time ){

        std::string timeVal = wstrToStr( _time );
        return atof( timeVal.c_str() ); // TODO: может быть источник ошибок если время как int64_t
    }
}
//=====================
// utils for RTI
...
class FederateAmbassadorImpl : public rti1516e::NullFederateAmbassador {

    rti_client_vega::RTIObjectsManager * m_RTIObjectsManagerPtr;

    rti_client_vega::SFederateState     * m_federateStatePtr;

    const std::string   m_synchPointNameForMasterFederate;

public:

    FederateAmbassadorImpl( rti_client_vega::RTIObjectsManager * _mgr, rti_client_vega::SFederateState * _RTICallbackFlags, const std::string _synchPointNameForMasterFederate );

    virtual ~FederateAmbassadorImpl() throw();

    /*
     * все обратные вызовы в модели HLA_IMMEDIATE выполняются асинхронно в ОТДЕЛЬНОМ ПОТОКЕ
     */
    // 4.4
